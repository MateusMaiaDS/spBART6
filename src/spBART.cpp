...
void Node::splineNodeLogLike(modelParam& data, arma::vec &curr_res){
        ...
        for(int k = 0; k < data.d_var;k++){
                ...
                if(ancestors(k)>0){
                        res_cov = res_cov + (1/data.tau_b)*B_.slice(k)*data.diag*B_t_.slice(k); // Do not penalise by tree number of trees factor
                }
                // Test elements
        ...
        for(int i = 0; i < t_nodes.size();i++){
                ...
                for(int j=0;j<data.d_var;j++){
                        ...
                        if(t_nodes[i]->ancestors(j)>0){
                                ...
                                arma::mat aux_precision_inv = arma::inv(t_nodes[i]->B_t.slice(j)*t_nodes[i]->B.slice(j)+((data.tau_b)/data.tau)*data.diag);
...
void updateTauB(Forest all_trees,
        ...
        double beta_count_total = 0;
        double beta_sq_sum_total = 0;
        ...
        for(int j = 0; j < data.d_var;j++){
                ...
                for(int t = 0; t< all_trees.trees.size();t++){
                        ...
                        for(int i = 0; i< t_nodes.size(); i++ ){
                                ...
                                if(t_nodes[i]->ancestors(j)>0){
                                        beta_sq_sum_total = beta_sq_sum_total + arma::as_scalar(t_nodes[i]->betas.col(j).t()*data.diag*t_nodes[i]->betas.col(j));
                                        beta_count_total = beta_count_total + t_nodes[i]->betas.n_rows;
                                }
                        }
                }
        }
...
Rcpp::List sbart(arma::mat x_train,
        ...
        arma::mat tau_b_post = arma::vec(n_post,arma::fill::zeros);
